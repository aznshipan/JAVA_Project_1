	...
	public void pomme_pop(int cpt) { //fait apparaitre des pomme sur la carte_A
		if (cpt % 2 == 0) {
			...
			do {
				...
				if(Terrain.getTerrain()[x1][y1][1] < Terrain.getEau()) {
					bool_A=true;
					break;
				}
			...
			if(Terrain.getTerrain()[x1][y1][1] >= Terrain.getEau() && Terrain.getTerrain()[x1][y1][1] < Terrain.contourRoche()) {
					...
					if(Terrain.getTerrain()[i][j][1] >= Terrain.getEau() && Terrain.getTerrain()[i][j][1] < Terrain.contourRoche() && Terrain.getTerrain()[i][j][2] == 0){
						...
						for(int m=0;m<Monde.getcarte_Ag().size();m++){
							if(Monde.getcarte_Ag().get(m) instanceof M){
								if(((M)Monde.getcarte_Ag().get(m)).getX() == i && ((M)Monde.getcarte_Ag().get(m)).getY() == j ){
									occupe = true; //La position est occupé
								}
							}else{
								if(((Braconnier)Monde.getcarte_Ag().get(m)).getX() == i && ((Braconnier)Monde.getcarte_Ag().get(m)).getY() == j ){
									occupe = true;
								}
							}
						}
	...
	public void depart_feu() {
		...
		for (int i=0;i<carte_Ab.size();i++) {
				for (int j=0;j<carte_Ag.size();j++) {
					cpt=0;
					if ((Terrain.getTerrain()[(carte_Ab.get(i)).getX()][(carte_Ab.get(i)).getY()][1])==Terrain.SolLave() 
